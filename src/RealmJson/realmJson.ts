 ...
 * @returns
 */
export const createRealmJson = async ({
  metaRealmPath,
  loadableRealmPath,
  collectionName,
}: RJCreateParams): Promise<RealmJson> => {
  ...
  // 3. Setup RealmJson
  const RealmJson: RealmJson = await initializeRealmJson({
    metaRealmPath,
    loadableRealmPath,
    collectionName,
  });
...
export const loadRealmJson = async ({
  metaRealmPath,
  loadableRealmPath,
  collectionName,
  shouldReloadRealm = true,
}: RJLoadJsonParams): Promise<RealmJson> => {
...
const initializeRealmJson = async ({
  metaRealmPath,
  loadableRealmPath,
  collectionName,
}: RJCreateParams): Promise<RealmJson> => {
  ...
  function _getAllJsonRows(): Realm.Results<RealmJsonRow & Realm.Object> {
    ...
    const allJsonRows: Realm.Results<RealmJsonRow & Realm.Object> =
      loadedJsonRealm.objects(genJsonSchemaName(collectionName));
   ...
   * @returns
   */
  function _getJsonRow(
    rowKey: string,
    create: boolean = true
  ): RealmJsonRow & Realm.Object {
    const loadedJsonRealm: Realm = loadRealmSync();
    // 1. Get json row
    let jsonRow: (RealmJsonRow & Realm.Object) | undefined =
      loadedJsonRealm.objectForPrimaryKey(
  ...
  function _createJsonRow(key: string): (RealmJsonRow & Realm.Object) | never {
    ...
    loadedJsonRealm.write(() => {
      newJsonRow = loadedJsonRealm.create(
        genJsonSchemaName(collectionName),
        {
          id: key,
          json: {},
...
const _saveJsonSchema = (
  ...
  const saveParams: SaveSchemaParams[] = [jsonSchema].map(
...
const _deleteJsonSchema = async ({
  metaRealmPath,
  loadableRealmPath,
  collectionName,
  reloadRealm,
}: RJDeletePropertiesParams): Promise<void> => {
