...
const TEST_NAME: string = 'DeleteJsonTest';
...
describe('createRealmJson', () => {
    ...
    it('Should load Json1', async () => {
        const loadedRealmJson: RealmJson = await loadRealmJson({ metaRealmPath: META_REALM_PATH1, loadableRealmPath: LOADABLE_REALM_PATH1, collectionName: JSON_NAME1, shouldReloadRealm: false });
        ...
        // DELETE JSON1
        await loadedRealmJson.deleteCollection();

        schemaNames1 = MetaRealm.getSchemaNames(META_REALM_PATH1, LOADABLE_REALM_PATH1);
        expect(schemaNames1.sort()).toEqual([ jsonSchemaName2 ].sort());
        ...
        expect(loadedSchemaNames1.sort()).toEqual([ jsonSchemaName2 ].sort());
    });
