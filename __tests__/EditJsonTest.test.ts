...
describe("createRealmJson", () => {
  ...
  it("Should save Json1 Schema to the LoadableRealm1 LoadableSchemas table", async () => {
    ...
    const schemaNames: string[] = MetaRealm.getSchemaNames(
      META_REALM_PATH1,
      LOADABLE_REALM_PATH1
    );
  ...
  it("Should load the LoadableRealm with Json1 schema", async () => {
    const loadableRealm: Realm = await MetaRealm.LoadableRealmManager.loadRealm(
      {
        metaRealmPath: META_REALM_PATH1,
        loadableRealmPath: LOADABLE_REALM_PATH1,
      }
    ...
    const loadedSchemaNames: string[] = loadableRealm.schema.map(
      (schema) => schema.name
    );
